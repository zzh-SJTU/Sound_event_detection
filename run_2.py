...
parser.add_argument("--augment", type=str, default='pitch')
...
wandb.config = {"seed": args.seed, "dropout":args.dropout, "width": args.width,"num_layers":args.num_layers }
...
class Runner(object):
    ...
    def train(self, config_file = 'configs/baseline.yaml', **kwargs):
        ...
        trainloader = torch.utils.data.DataLoader(
            dataset.TrainDataset(
                ...
                config["data"]["feature"],
                train_df,
                label_to_idx,augment =args.augment
            ),
    ...
    def evaluate(
        ...
        print('****************')
        wandb.log({'event_f1': event_based_results['f_measure'].values[0]})
        print(event_based_results['f_measure'].values[0] )
        ...
        wandb.log({"report" : result_quick_report})
        print("Quick Report: \n{}".format(
    ...
    def train_evaluate(
        self, config_file = 'configs/baseline.yaml', eval_feature = 'data/eval/feature.csv', eval_label = 'data/eval/label.csv', **eval_kwargs):
        experiment_path = self.train(config_file)
        self.evaluate(experiment_path, eval_feature , eval_label , **eval_kwargs)


if __name__ == "__main__":
    wandb.config = {"seed": args.seed,"dropout": args.dropout}
    wandb.init(config = args)
    runer = Runner(seed = args.seed)
    runer.train_evaluate()
